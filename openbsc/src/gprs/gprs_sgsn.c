...
static struct apn_ctx *apn_ctx_alloc(const char *ap_name, const char *imsi_prefix)
{
	struct apn_ctx *actx;
...
void apn_ctx_free(struct apn_ctx *actx)
{
	llist_del(&actx->list);
	talloc_free(actx);
}

struct apn_ctx *apn_ctx_match(const char *name, const char *imsi)
{
	struct apn_ctx *actx;
...
struct apn_ctx *apn_ctx_by_name(const char *name, const char *imsi_prefix)
{
	struct apn_ctx *actx;
...
struct apn_ctx *apn_ctx_find_alloc(const char *name, const char *imsi_prefix)
{
	struct apn_ctx *actx;

	actx = apn_ctx_by_name(name, imsi_prefix);
	if (!actx)
		actx = apn_ctx_alloc(name, imsi_prefix);
						...
						enum gsm48_gsm_cause *gsm_cause)
{
	...
	if (strlen(req_apn_str) == 0 && !allow_any_apn) {
		...
		llist_for_each_entry(pdp, &mmctx->subscr->sgsn_data->pdp_list, list) {
			...
			if (!llist_empty(&sgsn_apn_ctxts)) {
				apn_ctx = apn_ctx_match(req_apn_str, mmctx->imsi);
				/* Not configured */
				if (apn_ctx == NULL)
					continue;
			}
	...
	if (apn_ctx == NULL && selected_apn_str)
		apn_ctx = apn_ctx_match(selected_apn_str, mmctx->imsi);
